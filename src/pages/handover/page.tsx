...
export default function HandoverPage() {
  ...
  const handleCompleteHandover = async () => {
    ...
    if (booking?.id) {
      ...
      setIsUpdating(false);
      if (!error && data) {
        navigate('/parking', { state: { valet, booking: data, ...location.state } });
      } else {
        ...
        {step === 2 && (
          <div className="py-8">
            ...
            <div className="space-y-4 mb-6">
              <Card className="p-6">
                <div className="flex items-center gap-4">
                  <div className={`w-20 h-20 rounded-full border-4 flex items-center justify-center transition-all ${userSelfie ? 'border-[#66BD59] bg-[#66BD59]/10' : 'border-neutral-300 bg-neutral-50'
                    }`}>
                    {userSelfie ? (
              ...
              <Card className="p-6">
                <div className="flex items-center gap-4">
                  <div className={`w-20 h-20 rounded-full border-4 flex items-center justify-center transition-all ${valetSelfie ? 'border-[#66BD59] bg-[#66BD59]/10' : 'border-neutral-300 bg-neutral-50'
                    }`}>
                    {valetSelfie ? (
                      <HiCheckCircle className="w-10 h-10 text-[#66BD59]" />
                    ) : (
        ...
        {step === 3 && (
          <div className="py-6">
            ...
            <div className="space-y-3 mb-6">
              {checklistItems.map((item) => {
                ...
                return (
                  ...
                  >
                    <div className="flex items-center gap-4">
                      <div className={`w-12 h-12 rounded-xl flex items-center justify-center transition-all ${isChecked ? 'bg-gradient-to-br from-[#34C0CA] to-[#66BD59]' : 'bg-neutral-100'
                        }`}>
                      ...
                      <div className={`w-7 h-7 rounded-full border-2 flex items-center justify-center transition-all ${isChecked ? 'border-[#66BD59] bg-[#66BD59]' : 'border-neutral-300'
                        }`}>
            ...
            <Button
              ...
              disabled={!allChecked || !confirmHandover || isUpdating}
              fullWidth
              size="lg"
              icon="check"
            >
              {isUpdating ? 'Completing Handover...' : 'Complete Handover & Start Timer'}
            </Button>
          </div>
        )}
      </div>
