          ...
          return (
            ...
            >
              <div className={`relative ${isActive ? 'scale-110' : 'scale-100'} transition-transform duration-200`}>
                <Icon className={`w-6 h-6 ${isActive ? 'text-green-700' : 'text-neutral-500'}`} />
                {isActive && (
                  <div className="absolute -bottom-1 left-1/2 -translate-x-1/2 w-1 h-1 bg-green-700 rounded-full"></div>
                )}
              </div>
              <span className={`text-xs font-bold tracking-wide ${isActive ? 'text-green-700' : 'text-neutral-500'}`}>
                {item.label}
              </span>
            </button>
          );
